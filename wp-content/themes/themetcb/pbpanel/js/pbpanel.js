...
jQuery(document).ready(function($) {
    ...
    // $('.pbpanelAutocomplite').easyAutocomplete({
    //     url: function (phrase) {
    //         start = new Date().getTime();
    //         return '/wp-admin/admin-ajax.php?time='+start;
    //     },
    //     getValue: 'name',
    //     preparePostData: function (data,inputPhrase) {
    //         data = {};
    //         data.phrase = inputPhrase;
    //         data.action = 'AnalystNotebook';
    //         return data;
    //     },
    //     list: {
    //         onClickEvent: function($input) {
    //             console.log($($input).parent().next('input'));
    //         }
    //     },
    //     requestDelay: 400
    // });
    $('.pbpanelAutocomplite').autocomplete({
        source:
            function( request, response ) {
                start = new Date().getTime();
                $.ajax({
                    url: '/wp-admin/admin-ajax.php?time='+start,
                    dataType: "json",
                    data: {q: request.term,action: 'AnalystNotebook'},
        ...
        select:
            function( event, ui ) {
                ...
                $block = $this.parent().next('.itemsInfo');
                if(ui.item.attachment != undefined){
                    $block.children('img').attr('src',ui.item.attachment);
                } else {
                    $block.children('img').attr('src','/wp-content/themes/thecipherbrief/pbpanel/images/no_photo.png');
                }
                if(ui.item.author != undefined){
                    $block.children('span').text(ui.item.author);
                }
                if(ui.item.label != undefined){
                    $block.children('p').text(ui.item.label);
                }
            },
    });
